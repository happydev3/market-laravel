                    ...
                    <div class="card card-shadow">
                        <div class="card-body">
                            <div class="p-h-15 p-v-40">
                                <p style="padding: 14px" id="connectionId" hidden>{{$connection_id}}</p>
...
<script>
    $( document ).ready(function() {
       var connectionId= $('#connectionId').text();
       var qrPlaceholder = new QRCode("qrPlaceholder");
       qrPlaceholder.makeCode("https://dp.link/u/20/"+ connectionId);


        checkConnection();
        function checkConnection(){
            $.ajax({
                url: "{{route('store.ajax_check_connection')}}",
                success: function(data){
                    if(data == "true"){
                        window.location.replace("{{route('store.home')}}")
                    }
                },
                complete: function () {
